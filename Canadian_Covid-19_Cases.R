...
# Use the Logistic model to predict beyond the end of the full sample to 2 weeks past twice the point of inflection:

n_pred<- 2*gc_fit$vals$t_mid + 14
...
text(15,8000, cex=0.9,col="red", paste0("'Total capacity' = ", round(gc_fit$vals$k,0) , " cases"))
text(15,7200, cex=0.9,col="red", paste0("(95% C.I. = [", round(gc_fit$vals$k-1.96*gc_fit$vals$k_se,0), " , ",round(gc_fit$vals$k+1.96*gc_fit$vals$k_se,0), "] cases)"))
...
# Plot the full predicted time-path for the confirmed cases:
par(mfrow=c(1,1))
plot(pred, col="red",
     main="Projected Covid-19 Cases in Canada", 
     ylab="Cases", xlab="Day", 
     type="l")
...
text(60,1000, cex=0.8,font=3, paste0("Produced on ", today, "  (Data up to end of Day ", n_max,")"))
...
plot(Obs,capacity, main="Estimated 'Total Capacity'",
     ylab= "Total Capacity (Cases)", xlab="Sample Size (Days)", col="red",type="b", ylim=c(min(lower), max(upper)))
lines(Obs, lower, col="black",lty=2)
lines(Obs, upper, col="black",lty=2)
text(28.5,20000, cex=0.8,col="black", "95% Confidence Band")
