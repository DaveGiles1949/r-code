...
y2<- rggamma(n,sigma,1.5,2.333)    # length-biased gamma; k = 1.5
...
y3<- rggamma(n,sigma,2,2.0)    # length-biased gamma; k = 2.0
...
y4<- rggamma(n,sigma,3,1.6667)    # length-biased gamma; k = 3.0
...
legend(65, 0.6,legend = c(expression(paste("Weibull (k = 0.5): ",hat(lambda)[2])),
				...
				expression(paste("Weibull (k = 2.0): ",hat(lambda)[2])),
