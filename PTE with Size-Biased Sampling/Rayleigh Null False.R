...
# ****************************************************************************************
for (i in 1: nsamp) {
x<- rggamma(n,a,b,1)			   # Rayleigh with scale parameter sigma (crit values based on Ho True)
...
for (i in 1:m) {
y<- rggamma(n,a,b,k)		# Length-biased Rayleigh
...
par(mfrow=c(2,2))
plot(N,perc_bias_tilde,type="l",lwd=2,lty=2, ylim=c(-1,25),xlab="Sample Size (n)", ylab="%Bias", main=" ")
lines(N,perc_bias_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_bias_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 19,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
plot(N,perc_mse_tilde,type="l",lwd=2,lty=2, ylim=c(0,10),xlab="Sample Size (n)", ylab="%MSE", main=" ")
lines(N,perc_mse_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_mse_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 10,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
...
plot(N,perc_LINEX_risk_tilde,type="l",lwd=2,lty=2, ylim=c(0,20),xlab="Sample Size (n)", ylab="%LINEX Risk", main=" ")
