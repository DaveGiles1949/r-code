...
true_pbias_tilde[j]<- 100*(sqrt(2/N[j])*gamma((N[j]+1)/2)/gamma(N[j]/2) - 1)
perc_bias_hat[j]<- 100*(mean(sigma_hat)-sigma)/sigma
true_pbias_hat[j]<- 100*((1/sqrt(N[j]))*gamma((N[j]+1)/2)/gamma(N[j]/2) -1)
perc_bias_pte[j]<- 100*(mean(sigma_pte)-sigma)/sigma
perc_mse_tilde[j]<- 100*var(sigma_tilde)/sigma^2 + (perc_bias_tilde[j])^2/100
true_pmse_tilde[j]<- 100*2*(1-sqrt(2/n)*gamma((N[j]+1)/2)/gamma(N[j]/2) )
perc_mse_hat[j]<- 100*var(sigma_hat)/sigma^2 + (perc_bias_hat[j])^2/100
true_pmse_hat[j]<- 100*(3/2 - (2/sqrt(N[j]))*gamma((N[j]+1)/2)/gamma(N[j]/2)  )
...
par(mfrow=c(2,2))
plot(N,true_pbias_tilde,type="l",lwd=2,lty=2, ylim=c(-30,10),xlab="Sample Size (n)", ylab="%Bias", main=" ")
...
plot(N,true_pmse_tilde,type="l",lwd=2,lty=2, ylim=c(0,12),xlab="Sample Size (n)", ylab="%MSE", main=" ")
lines(N,true_pmse_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_mse_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 7,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)

plot(N,perc_abs_risk_tilde,type="l",lwd=2,lty=2, ylim=c(5,35),xlab="Sample Size (n)", ylab="%MAE", main=" ")
lines(N,perc_abs_risk_hat, type="l", lwd=3, lty=3, col="red")
lines(N,perc_abs_risk_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 25,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
...
legend(35, 11,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
