...
critt<- quantile(lambda,  probs = c(0.90, 0.95, 0.99))
#crit<- 0.724		# The critical values are specific to the Exponential distribution; the value of the parameter; and the sample size
crit<- critt[2]
...
if (lam1[i]<=crit) {		# Alter element from [1], [2], [3] for different sig. levels (10%, 5%, 1%)											
...
w<- 1.0	#  Shape parameter for LINEX loss function (+ve or -ve)
...
par(mfrow=c(2,2))
plot(N,true_pbias_tilde,type="l",lty=2, lwd=2,ylim=c(0,130),xlab="Sample Size (n)", ylab="% Bias", main=" ")
lines(N,true_pbias_hat, type="l", lwd=3, lty=3, col="red")
lines(N,perc_bias_pte, type="l", lty=1,lwd=2, col="blue")
legend(35, 60,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lty=c(2,3,1),lwd=c(2,3,2),ncol=1)

plot(N,true_pmse_tilde,type="l",lty=2, lwd=2,ylim=c(0,220),xlab="Sample Size (n)", ylab="%MSE", main=" ")
lines(N,true_pmse_hat, type="l", lty=3, lwd=3,col="red")
lines(N,perc_mse_pte, type="l", lty=1, lwd=2, col="blue")
legend(35, 100,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)

plot(N,perc_abs_risk_tilde,type="l",lty=2, lwd=2,ylim=c(0,140),xlab="Sample Size (n)", ylab="%MAE", main=" ")
lines(N,perc_abs_risk_hat, type="l", lty=3, lwd=3,col="red")
lines(N,perc_abs_risk_pte, type="l", lty=1, lwd=2,col="blue")
legend(35, 70,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)

plot(N,perc_LINEX_risk_tilde,type="l",lty=2,lwd=2, ylim=c(0,200),xlab="Sample Size (n)", ylab="%LINEX Risk", main=" ")
lines(N,perc_LINEX_risk_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_LINEX_risk_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 180,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
