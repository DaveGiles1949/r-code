...
# The length-biased Rayleigh is just the generalized Gamma density with a = sqrt(2*sigma^2), b=2,k=1.5
...
true_pbias_tilde[j]<- 100*(gamma(N[j]+0.5)/(sqrt(N[j])*gamma(N[j])) -1)
perc_bias_hat[j]<- 100*(mean(sigma_hat)-sigma)/sigma
true_pbias_hat[j]<- 100*(sqrt(2/(3*N[j]))*gamma(N[j]+0.5)/gamma(N[j]) - 1)
perc_bias_pte[j]<- 100*(mean(sigma_pte)-sigma)/sigma
perc_mse_tilde[j]<- 100*var(sigma_tilde)/sigma^2 + (perc_bias_tilde[j])^2/100
true_pmse_tilde[j]<- 100*2*(1-gamma(N[j]+0.5)/(sqrt(N[j])*gamma(N[j])))
perc_mse_hat[j]<- 100*var(sigma_hat)/sigma^2 + (perc_bias_hat[j])^2/100
true_pmse_hat[j]<- 100*(5/3-2*sqrt(2/(3*N[j]))*gamma(N[j]+0.5)/gamma(N[j])  )
...
par(mfrow=c(2,2))
plot(N,true_pbias_tilde,type="l",lwd=2,lty=2, ylim=c(-30,5),xlab="Sample Size (n)", ylab="%Bias", main=" ")
lines(N,true_pbias_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_bias_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, -6,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
plot(N,true_pmse_tilde,type="l",lwd=2,lty=2, ylim=c(0,6),xlab="Sample Size (n)", ylab="%MSE", main=" ")
lines(N,true_pmse_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_mse_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 3.3,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)

plot(N,perc_abs_risk_tilde,type="l",lwd=2,lty=2, ylim=c(0,30),xlab="Sample Size (n)", ylab="%MAE", main=" ")
...
plot(N,perc_LINEX_risk_tilde,type="l",lwd=2,lty=2, ylim=c(0,12),xlab="Sample Size (n)", ylab="%LINEX Risk", main=" ")
lines(N,perc_LINEX_risk_hat, type="l", lwd=3,lty=3, col="red")
lines(N,perc_LINEX_risk_pte, type="l", lwd=2,lty=1, col="blue")
legend(35, 12,legend = c(expression("Unweighted"),
                         expression("Length-Weighted"),
				 expression("Pre-Test")), 
                         box.col="white", col=c("black","red","blue"),lwd=c(2,3,2),lty=c(2,3,1),ncol=1)
