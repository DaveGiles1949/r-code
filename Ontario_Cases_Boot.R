...
# NOTE: "Day 1" for this analysis is 2 March, 2020
  ...
  t<- seq(1:length(tot))
  t7<- seq(1: (length(tot)+7))
  
 
  # Now use "Growthcurver" to fit Logistic Growth Model using latest sample
  
  gc_fit <- SummarizeGrowth(t, tot)               # Use these estimated coefficients as basis for point predections 
  ...
  for (ii in 1:n_boot) {
    resid <- tot - (gc_fit$vals$n0*gc_fit$vals$k / (gc_fit$vals$n0 +(gc_fit$vals$k - gc_fit$vals$n0)*exp(-t*gc_fit$vals$r)))
...
poi1<- round(18322+n_pred,0)    # Day 18322 is 2020-03-01. This is Day zero
class(poi1)<- "Date"
...
text(n_pred,tot[n_max]-2000, cex=0.7, col="red", max(low))
text(n_pred,tot[n_max], cex=0.7, col="red" , max(up))
text(n_pred,tot[n_max]-1000, cex=0.7, col="blue", fitted[n_pred])
text(n_pred-1,tot[n_max]-4000, cex=0.7, poi1)
